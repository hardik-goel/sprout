...
export function RewardFulfil() {
  ...
  return (
    <div className="flex min-h-screen flex-col pb-10">
      <PageHeader title="Reward" back="/parent/rewards" />
      <div className="flex flex-1 flex-col items-center px-5 text-center">
        ...
        <p className="text-muted">{reward.cost} points</p>

        {reward.redeemed ? (
          <div className="mt-6 flex items-center gap-2 rounded-full bg-gold/15 px-4 py-2 font-bold text-gold">
            <PartyPopper size={18} /> Redeemed {reward.redeemedAt ? `· ${shortDate(reward.redeemedAt.slice(0, 10))}` : ''}
          </div>
        ) : (
