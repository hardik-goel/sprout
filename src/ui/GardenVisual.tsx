import type { GardenStage } from '@/domain/types'
import { STAGE_EMOJI, STAGE_LABEL_KEY } from '@/domain'
import { t } from '@/i18n'
...
export function GardenVisual({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {flowers.length > 0 && (
        <div className="flex gap-1 text-xl" aria-label={t('garden.flowers.aria')}>
          {flowers.map((f, i) => (
      ...
      <div className="chip bg-glow/15 text-glow">{t(STAGE_LABEL_KEY[stage])}</div>
    </div>
  )
}
